import Vue from 'vue';
import './assets/styles/index.scss';
// 以下框架外部链接
import '@/assets/styles/index.scss';
import variables from './assets/styles/_variables.scss';
import HeaderItems from '@/components/HeaderItems.vue';
import setting from '@/../leaf-setting';

import {
  storeMixin,
  routerMixin,
  componentMixin,
  globalMixin,
  vueMixin
} from './instance';
import App from './App.vue';
import { obj } from './type';

export class Leaf {

  public router: any;
  public stores: any;
  public utils!: obj;
  public http!: obj;
  public headerSlot!: obj;
  public getMenuTree!: () => any;

  constructor() {
    // leaf对象prototype扩展
    globalMixin();
    storeMixin();
    routerMixin();
    componentMixin();
    vueMixin();
    // 初始化
    this._init();
  }
  
  private _init(this: any) {
    this._globalInit();
    this._storeInit();
    this._routerInit();
    this._componentInit();
    this._vueInit();
    this._depInit();
  }

  private _depInit() {
    // 自动加载store module
    const module = require.context('@/views', true, /\.module\.js$/);
    this.registerModule(module);
    
    // 自动加载路由
    const routers = require.context('@/views', true, /\.router\.ts$/);
    this.registerRouter(routers);
    
    // 自动加载svg
    const extSvgs = require.context('@/assets/img/svg', true, /\.svg$/);
    this.registerSvg(extSvgs); 

     // 自动加载utils, 如果文件名带下划线，则不提取
    const extUtils = require.context('@/utils', true, /\/[^_][^\/]*\.ts$/);
    this.registerUtils(extUtils); 

    // 自动加载全局组件
    const extComps = require.context('@/components', true, /\/[^_][^\/]*\.vue$/);
    this.registerComps(extComps);

    // 插入头部item
    this.headerSlot = HeaderItems;

    // 引入主题
    this.stores.dispatch('theme/setTheme', variables.theme);

    // 引入设置
    this.stores.dispatch('setting/setSetting', setting);
  }
 
  public register(options: obj = {}) {
    let {
      beforeCreate,
      created,
      mounted,
      httpConfig,
      getMenuTree,
   } = options;
    this.setHttpConfig(httpConfig);
    this.getMenuTree = getMenuTree;
    // 生命周期钩子函数
    this.beforeCreate = beforeCreate;
    this.created = created;
    this.mounted = mounted;
    return this;
  }
  /**
   * 启动项目
   */
  public async run(el: string = '#app') {
    try {
      this.beforeCreate && this.beforeCreate(this);
      const vueInstance: Vue = new Vue({
        router: this.router,
        store: this.stores,
        render: h => h(App),
      });
      this.created && this.created(this, vueInstance);
      vueInstance.$mount(el); 
      this.mounted && this.mounted(this, vueInstance);
    } catch(e) {
      console.error(e);
    }
    return this;
  }

}
export default new Leaf();
